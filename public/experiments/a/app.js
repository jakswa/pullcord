// Pullcord Client-Side JavaScript — Experiment A: Transit Pro
// Dark theme, CartoDB Dark Matter tiles, polished transit UI
...
class PullcordApp {
  ...
  initTrackerPage() {
    ...
    this.routeColor = this.data.route.color ? `#${this.data.route.color}` : '#3b82f6';
  ...
  initMap() {
    ...
    // CartoDB Dark Matter — dark tiles for Transit Pro theme
  ...
  drawRouteShapes() {
    ...
    Object.values(shapes).forEach(coordinates => {
      ...
      // Glow outline — route color at low opacity for depth on dark tiles
  ...
  updateETAs(predictions) {
    ...
    Object.entries(grouped).forEach(([headsign, preds]) => {
      ...
      // Direction header with SVG arrow
      const header = document.createElement('div');
      header.className = 'eta-group-header';
      header.innerHTML = `<svg class="eta-group-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="M12 5l7 7-7 7"/></svg> ${this.esc(headsign)}`;
      ...
      preds.forEach((pred) => {
        ...
        // SVG chevron for clickable rows
        const chevronSvg = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18l6-6-6-6"/></svg>';
        ...
        row.innerHTML = `
          ...
          ${isClickable ? `<div class="eta-chevron">${chevronSvg}</div>` : ''}
        `;
