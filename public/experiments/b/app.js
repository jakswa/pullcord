// Pullcord Client-Side JavaScript — Experiment B: Departure Board
// High-contrast, monospace numbers, infrastructure aesthetic
...
class PullcordApp {
      ...
      <div class="home-stop-card">
        ...
        <div class="home-stop-routes">
          ${stop.routes.map(route => `
            <a href="${window.__BASE_PATH__||""}/bus?route=${route}&stop=${stop.stop_id}" class="home-route-chip">
              ${route}
            </a>
          `).join('')}
        </div>
  ...
  initMap() {
    ...
    // CartoDB Positron — clean, minimal, pairs with departure board
    ...
    // Start centered on the stop at a useful zoom
  ...
  toggleMap() {
    const mapWrap = document.querySelector('.tracker-map-wrap');
    const etaSection = document.getElementById('eta-section');
    const toggleArrow = document.querySelector('.map-toggle-arrow');
    const toggleLabel = document.getElementById('map-toggle-label');
    
    this.mapCollapsed = !this.mapCollapsed;
    
    if (this.mapCollapsed) {
      mapWrap.classList.add('map-collapsed');
      etaSection.classList.add('etas-expanded');
      if (toggleArrow) toggleArrow.classList.add('collapsed');
      if (toggleLabel) toggleLabel.textContent = 'Show Map';
    } else {
      mapWrap.classList.remove('map-collapsed');
      etaSection.classList.remove('etas-expanded');
      if (toggleArrow) toggleArrow.classList.remove('collapsed');
      if (toggleLabel) toggleLabel.textContent = 'Hide Map';
      // Leaflet needs to recalculate size after show
      setTimeout(() => {
  ...
  updateETAs(predictions) {
    ...
    Object.entries(grouped).forEach(([headsign, preds]) => {
      ...
      // Direction header — departure board section label
      const header = document.createElement('div');
      header.className = 'eta-group-header';
      header.innerHTML = `<span class="eta-group-arrow">&rarr;</span> ${this.esc(headsign)}`;
      ...
      preds.forEach((pred) => {
        ...
        // Arrival time estimate — always show on departure board
        ...
        // Status — departure board uses dots + minimal text
        let statusHtml;
        if (tier === 'active') {
          const staleness = this.staleTier(pred.staleSeconds);
          statusHtml = staleness.html;
        } else if (tier === 'next') {
          statusHtml = '<span class="eta-next-dot"></span> Next run';
        } else {
        ...
        row.innerHTML = `
          <div class="eta-direction">
            <div class="eta-meta">
              ${statusHtml}
              <span style="color:#D1D5DB">·</span> ${timeStr}
            </div>
          </div>
          <div class="eta-time">
          ...
          ${isClickable ? '<div class="eta-chevron">&rsaquo;</div>' : ''}
        `;
  ...
  staleTier(s) {
    if (s < 45) {
      ...
      return { level: 'stale', html: `<span class="eta-stale-dot"></span> ~${m}m ago` };
    } else {
      const m = Math.floor(s / 60);
      return { level: 'lost', html: `<span class="eta-lost-dot"></span> Lost ${m}m ago` };
    }
  }
