...
// Mobile-first MARTA bus tracker — Warm & Playful edition
...
class PullcordApp {
  ...
  async searchStops(query, resultsList, resultsContainer, loadingDiv) {
    try {
      ...
      this.showError('Search hiccup — try again?', resultsList, resultsContainer, loadingDiv);
    }
  }

  async findNearbyStops(resultsList, resultsContainer, loadingDiv) {
    if (!navigator.geolocation) {
      this.showError('Location isn\'t available on this device', resultsList, resultsContainer, loadingDiv);
      return;
    }
    this.showLoading(loadingDiv, resultsContainer);
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          this.showError('Couldn\'t find nearby stops — try searching instead', resultsList, resultsContainer, loadingDiv);
        }
      },
      () => this.showError('Location access needed — check your browser settings', resultsList, resultsContainer, loadingDiv),
  ...
  displayStops(stops, resultsList, resultsContainer, loadingDiv) {
    ...
    if (stops.length === 0) {
      resultsList.innerHTML = '<div class="home-status">Hmm, no stops found. Try a different search?</div>';
      return;
    }
  ...
  showError(message, resultsList, resultsContainer, loadingDiv) {
    if (loadingDiv) loadingDiv.classList.add('hidden');
    if (resultsContainer) resultsContainer.classList.remove('hidden');
    if (resultsList) resultsList.innerHTML = `<div class="home-status" style="color:#E85D3A">${message}</div>`;
  }
  ...
  initMap() {
    ...
    // CartoDB Voyager — warm, friendly map tiles
    ...
    vehicles.forEach(v => {
      ...
      marker.bindPopup(`
        ...
        <small>Heading ${compass} · ${tier.level === 'live' ? 'Live' : 
          tier.level === 'delayed' ? 'Slow update' :
          `Last update ${Math.floor(v.staleSeconds/60)}m ago`}</small>
      `);
  ...
  staleTier(s) {
    if (s < 45) {
      ...
      return { level: 'delayed', html: '<span class="eta-delayed-dot"></span> Slow update' };
    } else if (s < 180) {
      const m = Math.floor(s / 60);
      return { level: 'stale', html: `<span class="eta-stale-dot"></span> ~${m}m since last update` };
    } else {
      const m = Math.floor(s / 60);
