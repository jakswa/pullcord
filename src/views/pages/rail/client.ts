...
function heroHTML(slug,data){var dirs=DORD.filter(function(d){return data.dirs[d]&&data.dirs[d].length});var quad=dirs.length>2;var grid=dirs.map(function(d){return herodir(d,data.dirs[d])}).join("");return '<a class="rail-hero rail-rv" href="/rail/'+slug+'"><div class="rail-hero-eyebrow mono">'+PIN+' your station</div><span class="rail-hero-star" role="button" aria-label="Unfavorite" data-slug="'+slug+'">'+STAR+'</span><h2 class="rail-hero-name">'+esc(data.name)+'</h2><div class="rail-hero-grid'+(quad?" is-quad":"")+'">'+grid+'</div><div class="rail-hero-foot mono">tap for full board \\u2197</div></a>'}
function emptyHeroHTML(){return '<div class="rail-hero-empty rail-rv"><div class="rail-hero-empty-star">\\u2606</div><div class="rail-hero-empty-title">no pinned stations yet</div><div class="rail-hero-empty-sub">tap the star on any station below to pin it here for one-glance departures.</div></div>'}
